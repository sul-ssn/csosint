...
- **SSRF-guard** (`csosint_common.netguard`, ТЗ §11.1): в recon идут только
  публичные IP; приватные/loopback/link-local (в т.ч. cloud-metadata
  `169.254.169.254`), ULA и reserved блокируются. Проверка — на фактическом IP
  **после** DNS-резолва (защита от DNS rebinding).
- **Строгая валидация ввода** (§11.2): `POST /scan` проверяет формат домена и
  публичность IP ещё на входе (422); приватный IP-таргет отклоняется сразу.
- **Rate-limiting** (§7): sliding-window лимитер на все `/api/v1` (лимит
  `RATE_LIMIT_PER_MINUTE`, дефолт 60/мин на клиента) → 429 при превышении.
- SQL-запросы параметризованы (SQLAlchemy); единственный `WITH RECURSIVE` в графе
  использует bind-параметр, без конкатенации ввода.

Роадмап ТЗ §8 пройден. Возможные следующие шаги — расширение alias-карты/CPE-словаря,
реальный bootstrap NVD, доменная верификация для публичного инстанса (§11.3).
