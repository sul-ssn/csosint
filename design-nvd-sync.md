# Дизайн: синк базы NVD (детализация §4.2 ТЗ)
...
- с бесплатным ключом: **50 / 30 сек** — ключ обязателен (§4.2 ТЗ);
...
- **Инкремент:** `lastModStartDate = last_success_cursor`, `lastModEndDate = now`.
  ...
  сутки (или чаще — NVD обновляется постоянно, но суточного хватает по ТЗ).
...
## 13. Связь с ТЗ и статус пакета документов

- Детализирует **§4.2** ТЗ; наполняет схему **§5** (`cve_records`,
...
1. `specification` — ТЗ v2 (общая архитектура, решения, роадмап).
